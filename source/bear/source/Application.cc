            ...
            .map<sys::Process::Builder>(
                [&environment, &output, &verbose, &force_wrapper, &force_preload](auto tuple) {
                    ...
                    auto builder = sys::Process::Builder(program)
                                       ...
                                       .add_argument(cmd::intercept::FLAG_LIBRARY)
                                       .add_argument(library)
                                       .add_argument(cmd::intercept::FLAG_WRAPPER)
                                       .add_argument(wrapper)
                                       .add_argument(cmd::intercept::FLAG_WRAPPER_DIR)
                                       .add_argument(wrapper_dir)
                                       .add_argument(cmd::intercept::FLAG_OUTPUT)
                                       .add_argument(output);
                    if (force_wrapper) {
        ...
        return rust::merge(program, output)
            .map<sys::Process::Builder>([&environment, &input, &config, &append, &verbose](auto tuple) {
                ...
                auto builder = sys::Process::Builder(program)
                                   ...
                                   .add_argument(cmd::citnames::FLAG_INPUT)
                                   .add_argument(input)
                                   .add_argument(cmd::citnames::FLAG_OUTPUT)
                                   .add_argument(output)
...
namespace bear {
    ...
    rust::Result<flags::Arguments> Application::parse(int argc, const char** argv) const
    {
        ...
        const flags::Parser citnames_parser("citnames", cmd::VERSION, { { cmd::citnames::FLAG_INPUT, { 1, false, "path of the input file", { cmd::intercept::DEFAULT_OUTPUT }, std::nullopt } }, { cmd::citnames::FLAG_OUTPUT, { 1, false, "path of the result file", { cmd::citnames::DEFAULT_OUTPUT }, std::nullopt } }, { cmd::citnames::FLAG_CONFIG, { 1, false, "path of the config file", std::nullopt, std::nullopt } }, { cmd::citnames::FLAG_APPEND, { 0, false, "append to output, instead of overwrite it", std::nullopt, std::nullopt } }, { cmd::citnames::FLAG_RUN_CHECKS, { 0, false, "can run checks on the current host", std::nullopt, std::nullopt } } });
